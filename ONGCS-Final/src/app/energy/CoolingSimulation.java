...
public class CoolingSimulation {
	...
	public CoolingSimulation(float tSuppliedByCRAC) {
		...
		this.COP = (float) (0.0068 * Math.pow(tSuppliedByCRAC, 2) + 0.0008
				* tSuppliedByCRAC + 0.458);
	}
	...
	public void setSingleServerCoolingValueGivenPowerConsumption(Server s,
			float power) {
	...
	public float getCOP(float temperature) {
		return (float) (0.0068 * Math.pow(temperature, 2) + 0.0008
				* temperature + 0.458);
	}

	public float getRackCoolingValueGivenInletTemperatureAndPowerValue(
			float inletTemperature, float powerValue) {
		float coefficientOfPerformance = (float) (0.0068
				* Math.pow(inletTemperature, 2) + 0.0008 * inletTemperature + 0.458);
