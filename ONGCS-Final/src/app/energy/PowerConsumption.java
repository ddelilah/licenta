...
public class PowerConsumption {
	...
	public float computeSingleServerPowerConsumptionGivenUtilization(Server s, float utilization) {
		return s.getIdleEnergy() + (MAXIMUM_POWER - s.getIdleEnergy()) * utilization;
	}
	
	public void setSingleServerPowerConsumptionGivenUtilization(Server s, float utilization) {
		if (utilization != 0) {
			float power = computeSingleServerPowerConsumptionGivenUtilization(s, utilization);
