...
//		
	...
	private boolean checkIfInstanceAlreadyAdded(VirtualMachine vmToCheck, List<VirtualMachine> toBeDeployedVmList){
		
		for(VirtualMachine vm: toBeDeployedVmList){
			System.out.println("Checking.... "+vm.getVmId() +" and "+ vmToCheck.getVmId());
  ...
  // ---------------------------------- CREATE COMMAND --------------------------------------------------------------------------------
		if (message.getCommand().equals("CREATE")) {
			if (message.getType() instanceof VirtualMachine) {
				...
				System.out
				.println("\n\n\n...........Preparing to create "+message.getNumberOfInstances()+
						" VMs...........");
...
// ---------------------------------- DEPLOY COMMAND --------------------------------------------------------------------------------
			...
			System.out
			.println("\n\n\n...........Preparing to deploy "+message.getNumberOfInstances()+
					" VMs...........");
			...
			for(int i=0; i<message.getNumberOfInstances(); i++){
				...
				for (VirtualMachine virtualM : taskList) {
					pos++;
					if (virtualM.getName().equals(vm.getName()) && virtualM.getState().equalsIgnoreCase(VMState.PENDING.getValue())) {
						vmToDeploy = virtualM;
						vmToDeploy.setState(VMState.DEPLOY.getValue());
						if(!checkIfInstanceAlreadyAdded(vmToDeploy,toBeDeployedVmList))
				...
				/* Check if we wanted to change a vm's state from SHUT_DOWN to DEPLOY*/
				pos =-1;
				if(!modifyNewlyCreated)
					for (VirtualMachine virtualM : taskList) {
						pos++;
						if (virtualM.getName().equals(vm.getName()) && !virtualM.getState().equalsIgnoreCase(VMState.DEPLOY.getValue())) {
							vmToDeploy = virtualM;
							vmToDeploy.setState(VMState.DEPLOY.getValue());
							if(!checkIfInstanceAlreadyAdded(vmToDeploy,toBeDeployedVmList))
								toBeDeployedVmList.add(vmToDeploy);
...
// ---------------------------------- SHUTDOWN COMMAND --------------------------------------------------------------------------------
			...
			System.out
			.println("\n\n\n...........Preparing to shutdown "+message.getNumberOfInstances()+
					" VMs...........");
			...
			for(int i=0; i<message.getNumberOfInstances(); i++){
				...
				/* Check if we wanted to change a vm's state from DEPLOY to SHUT_DOWN*/
				pos =-1;
				if(!modifyNewlyCreated)
					for (VirtualMachine virtualM : taskList) {
					pos++;
					if (virtualM.getName().equals(vm.getName()) && !virtualM.getState().equalsIgnoreCase(VMState.SHUT_DOWN.getValue())) {
						vm = virtualM;
						vm.setState(VMState.SHUT_DOWN.getValue());
						modifyNewlyCreated = true;
						System.out.println("Found in list");
				...
				for(int i=0; i<message.getNumberOfInstances(); i++) {
					...
					for(VirtualMachine virtualM: toBeDeployedVmList){
						...
						if(virtualM.getVmId() == vmToDelete.getVmId()){
							System.out.println((virtualM.getVmId() == vmToDelete.getVmId())+" "+virtualM.getVmId() +" "+vmToDelete.getVmId());
							deleteFromDeployedList = true;
							break;
						}}
