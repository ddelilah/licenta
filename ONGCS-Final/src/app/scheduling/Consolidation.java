...
public class Consolidation {
	...
	public void canMoveAllVMsSomewhereElse(List<VirtualMachine> VMs,
		...
		for (VirtualMachine selectedVm : sortedVMs) {
			...
			resultOfOBFD = obfd.findAppropriateServerForConsolidationStep(
					selectedVm, allocation);
		...
		if (canMove && allocation.entrySet().size() == numberOfVms) {
			...
			for (Entry<VirtualMachine, Server> entry : allocation.entrySet()) {
				...
				serverToBePlacedOn.setCorrespondingVMs(SchedulingUtil
	...
	public void tryToMoveAllVMsFromAServer(Server s, Charts chart) {
		...
		for (Server sr : allServersInDataCenter) {
			ServerPolicy serverPolicy = new ServerPolicy(p.SERVER_POLICY,
					false, sr);
			if (serverPolicy.checkServerUtilizationViolation(sr
					.getUtilization())) {
		...
		serverTaxonomyOnRack = consolidationUtil
				.serverCategory(serversOnTheSameRack);
		...
		for (Server s1 : serversOnTheSameRack) {
			List<VirtualMachine> virtualMachinesOnServersFromTheSameRack = s1
					.getCorrespondingVMs();
		...
		} else if (r.getState().equalsIgnoreCase("ON")
				&& rackPolicy.checkRackUtilizationViolation(r.getUtilization())) {
			System.out
					.println("[RACK POLICY IS VIOLATED => MIGRATE ALL VMS FROM THE UNDERUTILIZED SERVER ON RACK ]"
							+ r.getName()
							+ " "
							+ r.getUtilization()
	...
	public void consolidationOnDelete(List<VirtualMachine> vmsToBeDeleted,
			Charts chart, ChartAirflow chartAirflow, String algorithm) {
		...
		if (underUtilizedServerFromAllocationStep != null
				&& underUtilizedServerFromAllocationStep.getServerId() != 0
				&& !underUtilizedServerFromAllocationStep.getState()
		...
		if (underUtilizedServerFromAllocationStep != null
				&& underUtilizedRackFromAllocationStep.getRackId() != 0) {
			if (underUtilizedServerFromAllocationStep.getRack().getRackId() != underUtilizedRackFromAllocationStep
					.getRackId()) {
				// RECONSOLIDATION FOR RACKS
				if (underUtilizedRackFromAllocationStep != null
						&& !underUtilizedRackFromAllocationStep.getState()
								.equalsIgnoreCase(RackState.OFF.getValue())) {
					for (Server srv : underUtilizedRackFromAllocationStep
							.getServers()) {
						for (Server sr2 : resultOfServerAllocation) {
							if (srv.getServerId() == sr2.getServerId()) {
								srv.setCorrespondingVMs(sr2
										.getCorrespondingVMs());
					...
					// move all vms from that rack on other servers from other
					// racks
				...
				System.out
						.println("[UNDERUTILIZED SERVER IS ON THE UNDERUTILIZED RACK => CORNER CASE PREVIOUSLY CHECKED]");
				Thread.yield();
				try {
					Thread.sleep(1000);
				} catch (InterruptedException e) {
		...
		for (Server sr : allModifiedServers) {
			...
			for (Rack realloc : resultOfRackReallocation) {
				if (realloc.getRackId() == correspondingRack.getRackId()) {
					...
					correspondingRack
							.setCoolingValue(realloc.getCoolingValue());
			...
			if (sr.getState().equalsIgnoreCase("ON")
					&& serverPolicy.checkServerUtilizationViolation(sr
							.getUtilization())) {
				System.out
						.println("[SERVER POLICY IS VIOLATED => MIGRATE ALL VMS FROM SERVER TO OTHER SERVERS]");
				Thread.yield();
				try {
					Thread.sleep(1000);
				} catch (InterruptedException e) {
				...
				if (correspondingRack.getState().equalsIgnoreCase("ON")
						&& rackPolicy
								.checkRackUtilizationViolation(correspondingRack
										.getUtilization())) {
					Thread.yield();
					try {
						Thread.sleep(1000);
		...
		System.out.println("Migration efficiency: "
				+ mEff.computeMigrationEfficiency(numberOfReleasedNodes,
						numberOfSuccessfulMigrations));
		System.out.println("[CONSOLIDATION] #Released Nodes: "
				+ numberOfReleasedNodes);
		System.out.println("[CONSOLIDATION] #Migrations: "
				+ numberOfSuccessfulMigrations);
		Thread.yield();
		try {
			Thread.sleep(1000);
	...
	public void handleTheFailedVMs(Charts chart, ChartAirflow chartAirflow,
			String algorithm) {
		List<VirtualMachine> failedVMs = vmDAO.getAllVMsByState(VMState.FAILED
				.getValue());
		List<Rack> allRacks = rackDAO.getAllRacks();
		if (failedVMs.size() != 0) {
			switch (algorithm) {
