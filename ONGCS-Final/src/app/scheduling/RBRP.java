...
public class RBRP implements Serializable {
	...
	public Map<VirtualMachine, Server> placeVMsRackByRack(
			List<VirtualMachine> vmList, List<Rack> rackList) {
		...
		for (VirtualMachine vm : vmList) {
			...
			if (!onServerList.isEmpty()
					&& enoughResources(onServerList, vm, allocation)) {
	...
	public boolean enoughResources(List<Server> serverList,
			VirtualMachine vmToCheck, Map<VirtualMachine, Server> allocation) {
		Server s = new Server();
		for (Server server : serverList)
	...
	public Server getFirstOFFServer(Rack rack,
			Map<VirtualMachine, Server> allocation) {
